...
<?foreach ($arResult["SHOW_FIELDS"] as $FIELD):?>
	...
	{
		case "PASSWORD":
			?><input size="30" type="password" name="REGISTER[<?=$FIELD?>]" value="<?=$arResult["VALUES"][$FIELD]?>" autocomplete="off" class="bx-auth-input" placeholder="Пароль"/>
...
<?
		...
		case "CONFIRM_PASSWORD":
			?><input size="30" type="password" name="REGISTER[<?=$FIELD?>]" value="<?=$arResult["VALUES"][$FIELD]?>" autocomplete="off" placeholder="Подтверждение пароля"/><?
			break;
		...
		default:
			...
			?><input size="30" type="text" name="REGISTER[<?=$FIELD?>]" value="<?=$arResult["VALUES"][$FIELD]?>" placeholder="<?=getMessage('REGISTER_PLACEHOLDER_'.$FIELD);?>" />
            <?
...
<?if($arResult["USER_PROPERTIES"]["SHOW"] == "Y"):?>
	...
	<tr><td><?=$arUserField["EDIT_FORM_LABEL"]?>:<?if ($arUserField["MANDATORY"]=="Y"):?><span class="starrequired">*4</span><?endif;?></td>
        <td>
			<?$APPLICATION->IncludeComponent(
...
if ($arResult["USE_CAPTCHA"] == "Y")
{
	?>
		...
		<tr>
			<td></td>
			<td><input type="text" name="captcha_word" maxlength="50" value="" autocomplete="off" placeholder="<?=GetMessage("REGISTER_CAPTCHA_PROMT")?>:"/></td>
		</tr>
	<?
}
/* !CAPTCHA */
...
<p><span class="starrequired">*5</span><?=GetMessage("AUTH_REQ")?></p>
